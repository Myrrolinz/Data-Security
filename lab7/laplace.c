...
{
	double t;
	*seed = 2045.0 * (*seed) + 1; // 2045.0 = 2^11 + 1
	*seed = *seed - (*seed / 1048576) * 1048576; // 1048576 = 2^20
	t = (*seed) / 1048576.0; 
	t = a + (b - a) * t; // 生成[a,b]区间上的均匀分布随机数
	return t;
}
...
{
	double u1,u2, x;
	u1 = uniform_data(0.0, 1.0, seed); // 生成[0,1]区间上的均匀分布随机数
	u2 = uniform_data(0.0, 1.0, seed); // 生成[0,1]区间上的均匀分布随机数
	if (u1 < 0.5)	// 生成拉普拉斯分布随机数
	{
		x = beta * (log(2*u1)+u2);
	}
	else
